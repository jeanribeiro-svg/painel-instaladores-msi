...
function showPage(page, updateHash = true){
  const target = page === "midias" ? "midias" : "dashboard";
  const dash = $("page-dashboard");
  const mid = $("page-midias");

  if (dash) dash.classList.toggle("hidden-page", target !== "dashboard");
  if (mid) mid.classList.toggle("hidden-page", target !== "midias");

  document.querySelectorAll(".nav-link").forEach(a => {
    a.classList.toggle("active", a.dataset.page === target);
    a.setAttribute("aria-current", a.dataset.page === target ? "page" : "false");
  });

  if (updateHash) {
    const hash = target === "midias" ? "#midias" : "#dashboard";
    if (location.hash !== hash) history.pushState({page: target}, "", hash);
  }

  if (target === "midias" && !midiasLoaded) loadMidias();
}
...
function bindNavigation(){
  ...
  $("openMidiasFromCard")?.addEventListener("click", event => {
    event.preventDefault();
    showPage("midias", true);
    window.scrollTo({top: 0, behavior: "smooth"});
  });

  window.addEventListener("hashchange", () => {
    showPage(location.hash === "#midias" ? "midias" : "dashboard", false);
  });
