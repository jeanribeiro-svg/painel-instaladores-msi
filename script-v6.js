...
async function loadMidias() {
  try {
    ...
    catch(e){ console.warn("CSV da aba Midias não carregou; tentando Google Sheets JSONP.",e); result=await fetchMidiasGoogleJsonp(); }
    ...
    if(!midiasLoaded) renderMidiasEmpty("Não foi possível carregar a aba Midias.");
  }
}
